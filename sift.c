...
CvSeq* DenseSampling(IplImage*** pyr, int octvs, int intvls, int initStep, CvMemStorage* storage );
void zero_feature_oris( CvSeq* features);
...
double*** descr_hist( IplImage*, int, int, double, double, int, int ,struct feature* );
...
int sift_features( IplImage* img, struct feature** feat )
{
	return _sift_features( img, feat, SIFT_INTVLS, SIFT_SIGMA, SIFT_CONTR_THR,
							...
							SIFT_DESCR_HIST_BINS, INIT_STEP_DENSE_SAMPLING );
}
...
int _sift_features( IplImage* img, struct feature** feat, int intvls,
				   ...
				   int img_dbl, int descr_width, int descr_hist_bins, int nStepSize )
{
	IplImage* init_img;
	IplImage*** gauss_pyr;
	...
	//here we sample densely
	features = DenseSampling(gauss_pyr, octvs, intvls, nStepSize, storage);
	//CheckPos(features,400,300);
	...
	//zero_feature_oris(features);
	fprintf(stdout,"Computing SIFT descriptors for %d IP.\n",features->total);
...
CvSeq* DenseSampling(IplImage*** pyr, int octvs, int intvls, int initStep, CvMemStorage* storage )
{
	...
	for( o = 0; o < octvs; o++ ){
		...
		for( i = 1; i <= intvls; i++ ){
			...
			double sepFactor = pow(2.0,(o+(double)(i-1)/(double)intvls));

			//double hist_width = SIFT_DESCR_SCL_FCTR * sepFactor;
			//double radius = hist_width * sqrt(2.0) * ( 4 + 1.0 ) * 0.5 + 0.5;

			nStep =  (double)initStep*sepFactor/(double)SIFT_SIGMA;
			//printf("%f: %f %f %f\n",sepFactor,nStep,radius,radius/nStep);

			/*SIFT_IMG_BORDER*/
			for(r = SIFT_IMG_BORDER; r < (double)pyr[o][0]->height-SIFT_IMG_BORDER; r+=nStep){
				for(c = SIFT_IMG_BORDER; c < (double)pyr[o][0]->width-SIFT_IMG_BORDER; c+=nStep){

					feat = new_feature();
					ddata = feat_detection_data( feat );
					feat->img_pt.x = feat->x = c * pow(2.0,o);
					feat->img_pt.y = feat->y = r * pow(2.0,o);

					//if(feat->y > 300.0 || feat->x > 400.0 )
					//{
					//	printf("here");
					//}
					feat->ori = 0.0;
...
void CheckPos(CvSeq* features, double mx, double my)
{
	struct feature* feat;
	int i, n;
...
{
	struct feature* feat;
	int i, n = features->total;

	for( i = 0; i < n; i++ )
	{
		feat = (struct feature*) cvGetSeqElem(features,i);
		feat->ori = 0.0;
	}
}
...
void compute_descriptors( CvSeq* features, IplImage*** gauss_pyr, int d, int n)
{
	...
	for( i = 0; i < k; i++ )
	{
		...
		hist = descr_hist( gauss_pyr[ddata->octv][ddata->intvl], ddata->r,
			ddata->c, feat->ori, ddata->scl_octv, d, n ,feat);
...
double*** descr_hist( IplImage* img, int r, int c, double ori,
							double scl, int d, int n, struct feature* pFeat)
{
	double*** hist;
	int atLeastOnce = 0;
